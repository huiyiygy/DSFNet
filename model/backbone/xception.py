...
class SeparableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=0, dilation=1, bias=False, BatchNorm=None):
...
class Block(nn.Module):
    def __init__(self, inplanes, planes, reps, stride=1, dilation=1, BatchNorm=None, start_with_relu=True, grow_first=True, is_last=False):
        ...
        if grow_first:
            ...
            rep.append(SeparableConv2d(inplanes, planes, 3, 1, padding=dilation, dilation=dilation, BatchNorm=BatchNorm))
            rep.append(BatchNorm(planes))
            filters = planes

        for i in range(reps - 1):
            rep.append(self.relu)
            rep.append(SeparableConv2d(filters, filters, 3, 1, padding=dilation, dilation=dilation, BatchNorm=BatchNorm))
        ...
        if stride != 1:
            rep.append(self.relu)
            rep.append(SeparableConv2d(planes, planes, 3, 2, padding=1, BatchNorm=BatchNorm))
            rep.append(BatchNorm(planes))

        if stride == 1 and is_last:
            rep.append(self.relu)
            rep.append(SeparableConv2d(planes, planes, 3, 1, padding=1, BatchNorm=BatchNorm))
            rep.append(BatchNorm(planes))
...
class Xception(nn.Module):
    ...
    def __init__(self, output_stride=8, BatchNorm=nn.BatchNorm2d):
        ...
        self.conv3 = SeparableConv2d(256, 256, 3, stride=1, padding=exit_block_dilations[1], dilation=exit_block_dilations[1], BatchNorm=BatchNorm)
        self.bn3 = nn.BatchNorm2d(256)

        # do relu here
        self.conv4 = SeparableConv2d(256, 256, 3, stride=1, padding=exit_block_dilations[2], dilation=exit_block_dilations[2], BatchNorm=BatchNorm)
        self.bn4 = nn.BatchNorm2d(256)

        self.conv5 = SeparableConv2d(256, 256, 3, stride=1, padding=exit_block_dilations[3], dilation=exit_block_dilations[3], BatchNorm=BatchNorm)
...
if __name__ == "__main__":
    ...
    # (3, 512, 512)
    # output_stride=8 Flops:  4.98 GMac Params: 892.1 k
